  ...
  if (typeof window.ethereum !== "undefined") {
    ...
    try {
      ...
      const transaction_hash = await axios
        .get(`http://localhost:5000/api/transactions/details/${cert_hash}`)
        .then((res) => {
    ...
    >
      ...
      {showTransactionInfo ? (
        <div className="transaction__info__container">
          ...
          <p style={{ fontSize: 12, paddingRight: "10px" }}>
          ...
          <p className="mt-2">Smart Contract Address:</p>
          <p style={{ fontSize: 12, paddingRight: "10px" }}>
            {contractAddress}
          </p>
          <p className="mt-2">Timestamp:</p>
          <p style={{fontSize:12,paddingRight:"10px"}}>{timeStamp}</p>
        </div>
      ) : null}
    </form>
  );
